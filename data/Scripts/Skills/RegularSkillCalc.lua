...
-- SkillID: 2030, Blade Storm+ EXTRA (Enhancement/Mastery)
function ExtraDamageCalc(Class, InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)
    local OutDamage = 0
    if (Class == CLASS_ILLUSIONKNIGHT) then
        OutDamage = (InDamage * (200 + (Dexterity / 30))) / 100
    else
        OutDamage = InDamage  -- Fallback for other classes
    end
    return OutDamage
end